(function($) {
  ...
  function saveMarker(latlng) {
    ...
    if (mapLocation.latlng !== str) {
      ...
      setMessage(
        __('Your marker exact position is accepted'),
        __('Hit :link to save this position.', { button: '<a id="save-button-trigger" href="./">"' . __('Save') . '"</a>' }),
        'info'
      );
    }
  }
  ...
  function renderMap() {
    ...
    if (!mapLocation.latlng) {
      getAddressLatLng(mapLocation.address_line_1 + ', ' + mapLocation.zip + ' ' + mapLocation.city + ', ' + mapLocation.country.name, function(latlng) {
        addMarker(latlng);
        setMessage(
          __('Marker was set automatically'),
          __('If it doesn\'t match exact position of your location, feel free to drag it.'),
          'warning'
        );
        ...
        getAddressLatLng(mapLocation.city + ', ' + mapLocation.country.name, function(latlng) {
          addMarker(latlng);
          setMessage(
            __('Automatic marker position is not accurate'),
            __('Please drag the marker on your own to the desired location.'),
            'warning'
          );
          ...
          getAddressLatLng(mapLocation.country.name, function(latlng) {
            ...
            $.get('http://ipinfo.io/geo', function(geo) {
              addMarker(geo.loc);
              setMessage(
                __('It was not possible to set the marker automatically'),
                __('Please drag the marker on your own to the desired location.'),
                'warning'
              );
            }, 'jsonp');
      ...
      enableMap(false);
      setMessage(
        __('People shall be able to find your class location effortless'),
        __('Please :link of your class before reviewing it on the map.', {
          link: '<a href="../" title="">' . __('add detailed address of the location') .'</a>'
        }),
        'error'
      );
    }
